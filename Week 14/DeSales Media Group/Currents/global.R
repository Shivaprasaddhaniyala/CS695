...
# Load data set
Currentsdata <- readRDS("Currents.RDS")
df <- Currentsdata
...
large_clusters <- which(clust$csize > 5)
...
central %<>%
  mutate(label = ifelse(group < 2, "", as.character(central$nodes.g2.)))
...
#        Consumer Profile
#********************************************
#subdf <- subset(Currentsdata, city =='new york city' | city =='brooklyn')
dfrm <-table(df[,c("MESSAGE_ACTION", "USER_GENDER")])
